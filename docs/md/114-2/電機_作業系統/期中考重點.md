...
!!! danger
    ...
    | 策略                       | 核心想法               |
    |----------------------------|------------------------|
    | `LRU(Least Recently Used)` | 把最久沒用的換掉       |
    | `FIFO(First-In First-Out)` | 把最早放進來的換掉     |
    | `Random(隨機)`             | 隨機挑一個換掉         |
    | `Pseudo-LRU(近似 LRU)`     | 用較低硬體成本近似 LRU |
...
!!! danger
    ...
    | controller 類型                 | 控制什麼                |
    |---------------------------------|-------------------------|
    | `SSD / disk controller`         | 控制 SSD、硬碟等儲存裝置 |
    | `USB controller`                | 控制 USB 裝置           |
    | `network controller`            | 控制網路卡              |
    | `keyboard / mouse controller`   | 控制鍵盤、滑鼠輸入       |
    | `display / graphics controller` | 控制顯示或圖形輸出      |
...
| 名稱         | 作用                                                                    |
|--------------|-------------------------------------------------------------------------|
| `rw_mutex`   | 保護 shared data，讓 writer 獨占；也由第一個 reader 鎖住，防止 writer 進來 |
| `mutex`      | 保護 `read_count`，避免多個 reader 同時改 `read_count`                   |
| `read_count` | 記錄目前有幾個 reader 正在讀                                            |
...
| 錯法                                      | 為什麼錯                                                       |
|-------------------------------------------|----------------------------------------------------------------|
| 以為每個 reader 都要 `wait(rw_mutex)`     | 這樣 readers 就不能同時讀，失去 readers-writers problem 的目的  |
| 以為 `mutex` 是保護 shared data           | `mutex` 是保護 `read_count`；shared data 主要由 `rw_mutex` 控制 |
| 忘記第一個 reader 要 `wait(rw_mutex)`     | writer 可能在 reader 讀的時候進來寫                            |
| 忘記最後一個 reader 要 `signal(rw_mutex)` | writer 可能永遠進不來，造成 starvation 風險                     |
...
!!! danger
    ### starvation 可能是 deadlock 造成的嗎？
    不能這樣說，因為 Starvation = 某個 process/thread 長期甚至永遠等不到資源或執行機會。
