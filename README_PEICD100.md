...
- 2026-04-04：已改善 Mermaid 放大檢視的清晰度；`mermaid-zoom.js` 原本雖然搬進 viewer 的是原生 `svg`，但放大方式使用 `transform: scale(...)`，在 Mermaid 開啟 `htmlLabels`、節點內含 `foreignObject` 時，瀏覽器容易把內容當成已合成圖層一起放大而看起來像點陣圖。這次改為在 viewer 內透過 `peicd-image-viewer__canvas` 只負責平移，並讓 `svg` 本身隨縮放直接重設實際寬高後重新渲染，避免把向量圖單純當位圖放大；同時把 `自定義.css` 與 `mermaid-zoom.js` 的版本參數更新為 `20260404-2` / `20260404-1` 以降低快取吃舊檔的機率；`.gitignore` 已檢查，這次不需修改。
